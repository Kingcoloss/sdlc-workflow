...
if (!config.skipReview && devDone > 0) {
  ...
  const reviewBase = [
    ...
    '',
    'DEV SUMMARY:',
    JSON.stringify(devResults.map(r => ({ id: r.task_id, status: r.status, summary: r.summary })), null, 2),
    '',
...
if (!config.skipUatPlan && devDone > 0) {
  uat = await agent([
    ...
    JSON.stringify(plan.tasks.map(t => ({ id: t.id, title: t.title, test_scenarios: t.test_scenarios })), null, 2),
    '',
    'DEV RESULTS:',
    JSON.stringify(devResults.map(r => ({ id: r.task_id, status: r.status })), null, 2),
    '',
